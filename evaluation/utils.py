...
class AverageMeter(object):
    ...
    def __init__(self, masked=False, all_samples=False):
        self.reset(masked=masked, all_samples=all_samples)
        self.frame_ids = list()
    ...
    def reset(self,
        ...
        if masked:
            self.avg = np.zeros(n_interactions)
            if all_samples:
                self.sum = np.zeros((n_interactions, n_samples))
            else:
                self.sum = np.zeros(n_interactions)
            self.count = np.zeros(n_interactions)
        else:
            self.avg = 0
            self.sum = 0
            self.count = 0
    ...
    def update(self, val, n=1, mean_value=True, current_frame=None):
        self.val = val
        self.list.append(val)
        # For summation, change nan to 0.
        val = np.nan_to_num(val, nan=0.0)
        self.count += n

        if len(val.shape) == 2:
            self.sum += val * n[:, None]
            self.avg = self.sum / self.count[:, None]
        else:
            self.sum += val * n
            self.avg = self.sum / self.count
        if current_frame is not None:
            self.frame_ids.append(current_frame)
...
def mkdir_if_missing(input_path):
    ...
    try:
        ...
        print(f'Warning: cannot change permission of {folder}')
        pass
